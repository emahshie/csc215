...
def remove_comments_split_lines(s):
    """
      ...
      >>> src3 = 'F3\\n3B\\nC4\\n00\\n42\\n===\\n0200:8F\\n0210:42\\n'
      >>> remove_comments_split_lines(src3)
      ['F3', '3B', 'C4', '00', '42', '===', '0200:8F', '0210:42']
    """
...
def is_valid_mem_addr(s):
    """
      >>> is_valid_mem_addr('fa')
      True
      >>> is_valid_mem_addr('0200')
      True
    """
    if len(s) != 2 and len(s) != 4:
        return False
    if len(s) == 2 and is_hex_byte_str(s):
        return True
    if is_hex_byte_str(s[:2]) and is_hex_byte_str(s[2:]):
        addr = hex_byte2int(s[:2]) * 256 + hex_byte2int(s[2:])
        if addr <= 8192:
            return True
    return False
...
def split_and_validate_source(prog_src):
    """
      ...
      >>> split_and_validate_source(['a0', '4b', '===', '80:f7'])
      (['a0', '4b'], ['80:f7'])
      >>> split_and_validate_source(['A0', '4B', '===', '0200:F7', '0210:42'])
      (['A0', '4B'], ['0200:F7', '0210:42'])
    ...
    for num, line in enumerate(split[1]):
        parts = line.split(':')
        if not(is_valid_mem_addr(parts[0])) or not(is_hex_byte_str(parts[1])):
            raise ValueError(
...
def make_file_bytearray(prog_and_data):
    ...
    for i in range(8192 - num_bytes):
        byte_ints.append(0)
    for var in data:
        location_str, value = var.split(':') 
        # Turn location into a mem addr in decimal
        if len(location_str) == 2:
            location = hex_byte2int(location_str)
        else:  # location is 4 hex digits
            location = hex_byte2int(location_str[:2]) * 256
            location += hex_byte2int(location_str[2:])

        byte_ints[location] = hex_byte2int(value)
