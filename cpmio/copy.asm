...
BDERR   EQU     255             ; BDOS RETURN ERROR FLAGORG     TPA                 ; ASSEMBLE PROGRAM FOR TPA
...
DONE:   LDA     DRSAV        ; RESTORE INITIAL DRIVE
        STA     DRIVE
        JMP     RBOOT; READ A FILE FROM DISK INTO "BUFFR"
...
GETEX:  CALL    CCRLF              ; NORMAL EXIT
        ...
        RET; DISPLAY READ ERROR MESSAGE
REMSG:  CALL    TWOCR
        CALL    SPMSG
...
DRERR:  XRA     A               ; ELSE SET ZERO FLAG
        RET                    ; AND RETURN; WRITE A FILE FROM "BUFFR" TO DISK
...
PUTEX:  CALL    CCRLF
        CALL    CPDMA              ; RESTORE CP/M DMA
        RET; DISPLAY FILENAME.TYP FROM TRANSIENT FCB
SHOFN:  PUSH    B               ; SAVE TEMP STORE
        ...
        POP     H
        POP     B               ; RESTORE AND
        RET                     ; RETURN; CP/M I/O SUBROUTINES  16 AUG 82
